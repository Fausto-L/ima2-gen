...
export async function composeSpriteAtlas(input: SpriteAtlasComposeInput): Promise<{ manifest: SpriteGenManifest; report: SpriteAtlasReport }> {
  ...
  try {
    ...
    for (let row = 0; row < rows.length; row++) {
      ...
      for (let column = 0; column < plans[row].ordered.length; column++) {
        const index = plans[row].ordered[column];
        const plain = curation?.pixel_perfect === false;
        const framePath = join(runDir, "frames", state, `frame-${index}${plain ? ".plain" : ""}.png`);
        const buffer = await renderFrame(framePath, width, height, plans[row].transforms.get(index) ?? { rotate: 0, scale: 1, dx: 0, dy: 0, shx: 0, shy: 0, flipX: 0 });
    ...
    const manifest = parseSpriteGenManifest({ ...source, curation_applied: Boolean(curation), animation: { ...source.animation, columns, rows: Object.fromEntries(rows.map((state, row) => [state, { ...source.animation.rows[state], row, frames: layoutRows[state].length }])) }, frame_layout: { ...source.frame_layout, sheetWidth: columns * width, sheetHeight: Math.max(1, rows.length) * height, rows: layoutRows } });
