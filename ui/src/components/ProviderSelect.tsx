...
export function useProviderAvailability(): Record<Provider, ProviderAvailability> {
  ...
  const dashscopeKeyOk = keyStatus?.dashscope?.valid === true;
  ...
  return {
    ...
    atlascloud: {
      ok: atlascloudKeyOk,
      reason: atlascloudKeyOk ? "" : "Atlas Cloud API key required",
    },
    dashscope: {
      ok: dashscopeKeyOk,
      reason: dashscopeKeyOk ? "" : "DashScope API key required",
    },
  };
}
